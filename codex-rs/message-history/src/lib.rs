...
//!
//! The history is stored at `~/.runnly/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.runnly`.
...
pub async fn append_entry(
    ...
    // Resolve `~/.runnly/history.jsonl` and ensure the parent directory exists.
